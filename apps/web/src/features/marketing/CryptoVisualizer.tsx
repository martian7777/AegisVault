...
const STAGES: StageInfo[] = [
  {
    ...
    inputs: [
      ...
      { label: 'Secret Key (128-bit High-Entropy Device Key)', value: 'aegis-sec-7f9a8b1c4e2d3f0a9b8c7d6e5f4a3b2c' },
      { label: 'Cryptographic Salt (CSPRNG 256-bit)', value: 'e4f9b2c8...d3a7e1f0' },
    ],
    process: {
      label: 'Input Normalization & Buffer Isolation',
      detail: 'Loaded into ephemeral Uint8Array memory buffers; never touched by React state or DOM.',
    },
    outputs: [
      { label: 'Combined Key Derivation Vector', value: 'PWD || SK || SALT (Loaded in Web Worker Memory)' },
    ],
    securityNote: 'Defense Against GPU / ASIC Brute Force: Dual-factor input provides an extra 128-bits of unguessable entropy.',
  },
  {
    id: 2,
    ...
    outputs: [
      { label: 'Master Key (MK - 256-bit CryptoKey)', value: '0x9a8f21bc... (Non-Extractable CryptoKey handle)', isEncrypted: true },
    ],
    securityNote: 'Worker Isolation: Even if an XSS vulnerability existed in the DOM, the raw master key buffer cannot be read.',
  },
  {
    id: 3,
    ...
    process: {
      ...
      detail: 'Generates mathematically distinct encryption and authentication keys from the root seed.',
    },
    outputs: [
      { label: 'K_enc (AES-256-GCM Wrapping Key)', value: 'Key-Wrap Key for Item Envelope Keys' },
      { label: 'K_auth (HMAC-SHA256 Verifier Key)', value: 'Constant-time login verification without exposing MK' },
    ],
    securityNote: 'Domain Separation: Ensures encryption operations never leak mathematical clues regarding auth verifiers.',
  },
  {
    id: 4,
    ...
    inputs: [
      { label: 'Item Plaintext Data', value: '{"service":"Github","username":"alice","password":"••••"}' },
      { label: 'Random Item Key (CSPRNG)', value: 'Unique 256-bit Key Generated per Item' },
      { label: 'Authenticated Additional Data (AAD)', value: '{"id":"item-042","version":1,"type":"login"}' },
    ],
    process: {
    ...
    outputs: [
      { label: 'Ciphertext Payload', value: 'f72a901e...49b2 (Authenticated 128-bit GCM Tag)', isEncrypted: true },
      { label: 'Wrapped Item Key', value: 'ae89012f... (Wrapped with K_enc)', isEncrypted: true },
    ],
    securityNote: 'Envelope Security: Compromise of an individual item key never compromises the remainder of the vault.',
  },
  {
    id: 5,
    ...
    inputs: [
      { label: 'Vault Item Records', value: 'Array of { id, type, wrappedKey, ciphertext, iv, aad }' },
    ...
    process: {
      ...
      detail: 'Stored strictly within the sovereign browser sandbox with full offline availability.',
    },
    outputs: [
      { label: 'Local Vault Status', value: 'Encrypted at Rest • Zero Telemetry Transmitted • Sovereign' },
    ],
    securityNote: 'Breach Immunity: Even physical extraction of the raw IndexedDB database yields zero readable credentials.',
  },
];
      ...
      <div className="marketing-container">
        <div className="section-header">
          ...
          <p className="section-description">
            Explore how AegisVault transforms your master credentials into multi-layered envelope-encrypted
            sovereign records with zero server exposure.
          </p>
        </div>
          ...
          <div className="visualizer-display-box">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBottom: '1.25rem' }}>
              <div>
                ...
                <h3 style={{ fontSize: '1.45rem', marginTop: '0.5rem', color: 'var(--text-title)' }}>
                  {currentStage.title}
                </h3>
              </div>
            ...
            <div className="crypto-flow-diagram">
              ...
              <div className="flow-node" style={{ borderColor: 'var(--cyan-border)' }}>
                <div className="flow-node-title">
                  ...
                  <span style={{ fontSize: '0.75rem', color: 'var(--cyan-primary)' }}>[ENGINE]</span>
                </div>
                <div style={{ marginTop: '0.5rem' }}>
                  <div style={{ fontWeight: 700, fontSize: '0.9rem', color: 'var(--text-title)', marginBottom: '0.35rem' }}>
              ...
              <div className="flow-node">
                <div className="flow-node-title">
                  ...
                  <span style={{ fontSize: '0.75rem', color: 'var(--emerald-primary)' }}>[SECURED]</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  {currentStage.outputs.map((out, i) => (
                    <div key={i}>
                      ...
                      <div
                        ...
                        style={out.isEncrypted ? { color: '#34d399', borderColor: '#065f46' } : undefined}
                      >
                        {out.value}
                      </div>
                    </div>
            ...
            >
              ...
              <span style={{ fontSize: '0.88rem', color: 'var(--text-title)' }}>
                <strong style={{ color: 'var(--cyan-primary)' }}>Cryptographic Guarantee:</strong> {currentStage.securityNote}
              </span>
            </div>
          </div>
        </div>
